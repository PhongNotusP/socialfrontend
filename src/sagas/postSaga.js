...
function* getPosts() {
  yield call(setPostLoading);
  try {
    const response = yield RequestService.get("/api/stories");
    // data = body cua response
    // Status = status cua response
    // header = header cua response
    // console.log(response.data);
    yield put({ type: types.SET_POSTS, payload: response.data.data });
  } catch (error) {
